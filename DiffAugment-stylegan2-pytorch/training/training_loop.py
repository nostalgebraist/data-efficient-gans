...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(2048 // training_set.image_shape[2], 7, 16)
    gh = np.clip(2048 // training_set.image_shape[1], 4, 16)
    ...
    # Load data.
    images, labels, txts = zip(*[training_set[i] for i in grid_indices])
    return (gw, gh), np.stack(images), np.stack(labels), txts
...
def training_loop(
    ...
    for name, module, opt_kwargs, reg_interval, scaler, reg_scaler in [('G', G, G_opt_kwargs, G_reg_interval, loss.G_scaler, loss.Greg_scaler), ('D', D, D_opt_kwargs, D_reg_interval, loss.D_scaler, loss.Dreg_scaler)]:
        if hasattr(module.mapping, 'text_encoder') and module.mapping.text_encoder is not None:
            param_groups = [
                {
            ...
            param_groups = [{"params": list(module.parameters())}]
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, txt=txt, txt_gain=txt_gain, noise_mode='const').cpu() for z, c, txt in zip(grid_z, grid_c, grid_txt)]).numpy()
