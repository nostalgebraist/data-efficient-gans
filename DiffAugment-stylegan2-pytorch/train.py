...
def setup_training_loop_kwargs(
    ...
    args.network_snapshot_ticks = 100 # snap
    ...
    args.G_kwargs = dnnlib.EasyDict(class_name='training.networks.Generator', z_dim=z_dim, w_dim=latent_size, mapping_kwargs=dnnlib.EasyDict(text_kwargs = dnnlib.EasyDict(), layer_features=z_dim), synthesis_kwargs=dnnlib.EasyDict())
    ...
    if text:
        ...
        if const_text_gain:
            args.G_kwargs.synthesis_kwargs.const_text_gain = True
            args.D_kwargs.block_kwargs.const_text_gain = True
    ...
    args.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.StyleGAN2Loss', r1_gamma=spec.gamma, use_amp=amp==True)
...
@click.option('--fmax', type=int, metavar='INT')
@click.option('--fmaps', type=float)
...
@click.option('--const-text-gain', type=bool, metavar='BOOL')
